...
arguments (Input)
    ...
    transmitFoci(:,:) ZBP.RCATransmitFocus % [receive_event_count, num_data_frame]
...
receiveElementCount = size(transmitFoci, 1);
numDataFrame = size(transmitFoci, 2);

for i = 1:numDataFrame
    assert(all([transmitFoci(:, i).transmit_receive_orientation] == transmitFoci(1, i).transmit_receive_orientation), ...
...
for n = 1:numel(transmitFoci)
    [i, j] = ind2sub(size(transmitFoci), n);
