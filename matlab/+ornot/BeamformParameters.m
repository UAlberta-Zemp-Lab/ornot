classdef BeamformParameters
    ...
    properties
        ...
        transducer_transform_matrices(1,16,:) single
    ...
    methods
        ...
        function bytes = ToBytes(bp)
            ...
            header.string_count = size(1, bp.strings);
    ...
    methods (Static)
        ...
        function bp = FromV2V3Bytes(bytes, major)
            ...
            if major == 3
                ...
                matrices = typecast(bytes(uint32(header.transducer_transforms_offset) + (1:(4*16*prod(bp.transducer_tile_count)))), 'single');
                bp.transducer_transform_matrices = reshape(matrices, 1, 16, []);
            ...
            elseif major == 2
                ...
                bp.transducer_transform_matrices(:,:,1) = header.transducer_transform_matrix;
            end
            ...
            if header.acquisition_parameters_offset >= 0
                switch header.acquisition_mode
                    ...
                    case ZBP.AcquisitionKind.EPIC_FORCES
                        ...
                        bp.transmit_foci = createArray([bp.receive_event_count, section_count], "ZBP.RCATransmitFocus");
                        for i = 1:section_count
            ...
            if header.major == 2 && header.minor == 1
                % TODO(rnp): need to read an offset at the end of the base V2 header and use that
                % to read an (1,bp.raw_data_dimension(3)) array of singles. that should be added
                % to bp.time_delays
