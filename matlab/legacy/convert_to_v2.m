...
filesets = [
    get_filesets_by_ext(directories(1), ".bpr");
    get_filesets_by_ext(directories(2), ".toml");
    get_filesets_by_ext(directories(3), ".bpr");
    ];
...
function filesets = get_filesets_by_ext(directory, ext)
listing = dir(directory);
filesets = string.empty();
for i = 1:numel(listing)
    [~, name, e] = fileparts(listing(i).name);
    if strcmpi(e, ext)
        filesets = [
            filesets;
            directory, name;
