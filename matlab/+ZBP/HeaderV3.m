...
classdef HeaderV3
	...
	methods
		function bytes = toBytes(obj)
			...
			bytes(125:128) = typecast(obj.contrast_data_flags(:),             'uint8');
			bytes(129:132) = typecast(obj.contrast_parameters_offset(:),      'uint8');
			bytes(133:136) = typecast(obj.emission_descriptors_offset(:),     'uint8');
			bytes(137:140) = typecast(obj.time_delays_offset(:),              'uint8');
			bytes(141:144) = typecast(obj.demodulation_frequencies_offset(:), 'uint8');
	...
	methods (Static)
		function out = fromBytes(bytes)
			...
			out.contrast_data_flags(:)             = typecast(bytes(125:128), 'uint32');
			out.contrast_parameters_offset(:)      = typecast(bytes(129:132), 'int32');
			out.emission_descriptors_offset(:)     = typecast(bytes(133:136), 'int32');
			out.time_delays_offset(:)              = typecast(bytes(137:140), 'int32');
			out.demodulation_frequencies_offset(:) = typecast(bytes(141:144), 'int32');
