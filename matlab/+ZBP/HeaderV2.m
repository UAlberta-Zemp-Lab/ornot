...
classdef HeaderV2
	...
	methods (Static)
		function out = fromBytes(bytes)
			...
			out.raw_data_kind(:)                 = typecast(bytes(33:36),   'uint32');
			out.raw_data_offset(:)               = typecast(bytes(37:40),   'int32');
			out.raw_data_compression_kind(:)     = typecast(bytes(41:44),   'uint32');
			out.decode_mode(:)                   = typecast(bytes(45:48),   'uint32');
			out.sampling_mode(:)                 = typecast(bytes(49:52),   'uint32');
			...
			out.acquisition_mode(:)              = typecast(bytes(165:168), 'uint32');
			out.acquisition_parameters_offset(:) = typecast(bytes(169:172), 'int32');
			out.contrast_mode(:)                 = typecast(bytes(173:176), 'uint32');
