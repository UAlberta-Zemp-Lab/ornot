...
class ZBP:
	...
	class HeaderV3:
		...
		def from_bytes(cls, bytes):
			...
			result.contrast_parameters_offset       = struct.unpack_from('<1l', bytes, 124)[0]
			result.emission_descriptors_offset      = struct.unpack_from('<1l', bytes, 128)[0]
			result.time_delays_offset               = struct.unpack_from('<1l', bytes, 132)[0]
			result.demodulation_frequencies_offset  = struct.unpack_from('<1l', bytes, 136)[0]
			result.transducer_transforms_offset     = struct.unpack_from('<1l', bytes, 140)[0]
			result.string_count                     = struct.unpack_from('<1L', bytes, 144)[0]
			result.string_table_offset              = struct.unpack_from('<1l', bytes, 148)[0]
			return result
		...
		def to_bytes(self):
			...
			struct.pack_into('<1l', result, 124,  self.contrast_parameters_offset)
			struct.pack_into('<1l', result, 128,  self.emission_descriptors_offset)
			struct.pack_into('<1l', result, 132,  self.time_delays_offset)
			struct.pack_into('<1l', result, 136,  self.demodulation_frequencies_offset)
			struct.pack_into('<1l', result, 140,  self.transducer_transforms_offset)
			struct.pack_into('<1L', result, 144,  self.string_count)
			struct.pack_into('<1l', result, 148,  self.string_table_offset)
			return result
